...
export const Retry = async <T extends any[], R>(p: RetryFn<T, R>, ...params: T): Promise<R> => {
  let lastErr: unknown = new Error(`operation failed after ${config.Retry.MaxAttempts} attempts`)
  ...
  for (let attempts = 0; attempts <= config.Retry.MaxAttempts; attempts++) {
    ...
    if (attempts !== 0) {
      ...
      delay = Math.min(delay * config.Retry.ExponentialBackOff, config.Retry.MaxTimeout)
    }
