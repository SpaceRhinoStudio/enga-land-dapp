...
type WrappedUnPromise<P> = P extends Promise<infer T> ? T : { [K in keyof P]: UnPromise<P[K]> }

export type UnPromise<P> = P extends WrappedUnPromise<unknown> ? P : WrappedUnPromise<P>
...
export interface StorageAPI {
  write(key: string, value: string, options?: StorageAPIWriteOptions): Promise<void>
...
export interface CacheService {
  ...
  getDefault<T = unknown>(key: string, initializer: T): Promise<T | UnPromise<T>>
  ...
  observe<T = unknown>(key: string, initializer?: T): ReplaySubject<T | UnPromise<T>>
}
