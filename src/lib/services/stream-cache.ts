...
export class StreamCache {
  ...
  private _makeObservable<T>(key: string) {
    ...
    zip(this.observables[key]! as ReplaySubject<T>, this.controls[key]! as Control<T>).subscribe(
      ([data, control]) => {
        this.observables[key]?.next(_.isFunction(control) ? control(data) : control)
      },
    )
  }
