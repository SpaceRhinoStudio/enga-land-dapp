...
export class BrowserLocalStorageAPI implements StorageAPI {
  ...
  private async _write(key: string, value: any): Promise<void> {
    Window$.subscribe(w =>
      w.localStorage.setItem(config.BrowserLocalStorageCacheKey.concat(key), value),
    )
  }
  ...
  public async read(key: string): Promise<string | null> {
    return localStorage.getItem(config.BrowserLocalStorageCacheKey.concat(key))
  }
  ...
  public async getAllKeys(): Promise<string[]> {
    return Object.keys(localStorage).map(key => key.replace(config.BrowserLocalStorageCacheKey, ''))
  }

  public observe(key: string): ReplaySubject<string | null> {
    const observables = this.memoryCache.get<ObservablesCache>(this.observablesMemoryCacheKey)
